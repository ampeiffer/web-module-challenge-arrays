...
function getFlavorByIndex(originalFlavors, num){
    let selectCity = [...originalFlavors];
    return (selectCity[num]);
...
function removeFlavorByName(taskSixArray, naughtyFlavor){
    for(let i=0; i<taskSixArray.length; i++){
        if(naughtyFlavor===taskSixArray[i]){
            taskSixArray.splice(i, 1)
        }else continue
    }return taskSixArray
}
